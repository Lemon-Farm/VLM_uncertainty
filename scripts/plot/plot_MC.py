...
DEFAULT_ID_PATH = Path("outputs/mc_dropout/mc_dropout_uncertainty_ID.csv")
DEFAULT_OOD_PATH = Path("outputs/mc_dropout/mc_dropout_uncertainty_OOD.csv")
DEFAULT_OUTPUT_DIR = Path("reports/figures")
UNCERTAINTY_COLUMN = "caption_uncertainty"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot MC-dropout uncertainty diagnostics.")
...
def load_uncertainty(path: Path) -> np.ndarray:
    ...
    if UNCERTAINTY_COLUMN not in dataframe.columns:
        raise ValueError(f"{path} does not contain '{UNCERTAINTY_COLUMN}'.")
    return dataframe[UNCERTAINTY_COLUMN].dropna().to_numpy(dtype=np.float64)


def plot_violin(id_scores: np.ndarray, ood_scores: np.ndarray, output_dir: Path) -> Path:
    output_path = output_dir / "mc_dropout_caption_uncertainty_violin.png"
    ...
    ax.set_ylabel("Caption uncertainty")
    ax.set_title("MC-dropout Caption Uncertainty")
...
def plot_roc(
    ...
    output_path = output_dir / "mc_dropout_caption_uncertainty_roc.png"
    ...
    ax.set_title("OOD ROC by Caption Uncertainty")
